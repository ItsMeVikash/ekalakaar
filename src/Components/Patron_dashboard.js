...
export default function Patron_dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let artistinitialpage = await fetch(
        `http://ekalakaar.com:4000/api/patron/${localStorage.getItem("_id")}`,
        {
          method: "GET",
          headers: {
    ...
    fetchData().then((d) => {
      ...
      setLocation(totaldata.patron.location);
      setWebsiteLink(totaldata.patron.website_link);
    });
  }, [artist_email]);
  ...
  useEffect(() => {
    let getLatestOpp = async () => {
      let data = await fetch(
        `http://ekalakaar.com:4000/api/users/opportunity/recentlyPostedOpportunities/${localStorage.getItem(
          "_id"
        )}`
      );
  ...
  useEffect(() => {
    let recentApplied = async () => {
      let data = await fetch(
        "http://ekalakaar.com:4000/api/users/opportunity/recentlyAppliedArtist"
      );
      let json = await data.json();
  ...
  useEffect(() => {
    let recentHired = async () => {
      let data = await fetch(
        "http://ekalakaar.com:4000/api/users/opportunity/recentlyHiredArtist"
      );
      let json = await data.json();
