...
export default function Artist_viewdetailopp() {
  ...
  useEffect(() => {
    async function fetchData() {
      let opportunityAllData = await fetch(
        "http://ekalakaar.com:4000/api/getAllOpportunities"
      );
  ...
  const handlebookmark = async () => {
    ...
    let user = await fetch(
      `http://ekalakaar.com:4000/api/userdetail/${localStorage.getItem("_id")}`
    );
    user = await user.json();
    console.log(user);

    let result = await fetch(
      `http://ekalakaar.com:4000/api/users/opportunity/${params.id}/${user.email}`,
      {
        method: "POST",
        headers: {
  ...
  let postOwner = async () => {
    ...
    let data = await fetch('http://ekalakaar.com:4000/api/getUserByEmail', {
      method: 'GET',
